...
export function buildStatusSnapshot(cwd, options = {}) {
  ...
  const jobs = sortJobsNewestFirst(filterJobsForSession(listJobs(workspaceRoot), options.sessionId));
...
export function resolveResultJob(cwd, reference, sessionId = null) {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const jobs = sortJobsNewestFirst(reference ? listJobs(workspaceRoot) : filterJobsForSession(listJobs(workspaceRoot), sessionId));
...
export function resolveCancelableJob(cwd, reference, sessionId = null) {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const jobs = sortJobsNewestFirst(listJobs(workspaceRoot));
  ...
  const sessionScopedActiveJobs = filterJobsForSession(activeJobs, sessionId);
